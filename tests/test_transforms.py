"""Unit tests for datasets/transforms.py — output shape/dtype/range must be
correct for both pipelines, since a mistake here silently corrupts every
downstream training signal rather than raising an obvious error."""
...
def test_eval_transform_is_deterministic(synthetic_image):
    """Unlike the train pipeline, eval must contain no randomness — the same
    image must always resize/normalize to exactly the same tensor, since
    validation/test metrics must be reproducible run-to-run."""
    data_cfg = DataConfig(image_size=224)
    aug_cfg = AugmentationConfig()
    transform = build_eval_transforms(data_cfg, aug_cfg)
...
def test_eval_transform_normalizes_to_roughly_zero_mean(synthetic_image):
    """Sanity check the normalization actually ran (rather than, say, a
    misconfigured pipeline silently skipping it): a uniformly random image
    normalized with ImageNet mean/std should land roughly centered on 0,
    not still in the raw [0, 255] range."""
    data_cfg = DataConfig(image_size=224)
    aug_cfg = AugmentationConfig()
    transform = build_eval_transforms(data_cfg, aug_cfg)
    ...
    assert output.mean().abs().item() < 3.0  # nowhere near the raw ~127 midpoint of [0, 255]


def test_train_transform_produces_varied_output_across_calls(synthetic_image):
    """The train pipeline's whole purpose is randomized augmentation — two
    calls on the same image should not produce identical tensors."""
    data_cfg = DataConfig(image_size=224)
    aug_cfg = AugmentationConfig()
    transform = build_train_transforms(data_cfg, aug_cfg)
