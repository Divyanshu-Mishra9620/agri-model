"""Unit tests for engine/losses.py — every configurable loss variant must
produce a finite, correctly-shaped scalar, and FocalLoss must reduce to
plain cross-entropy when gamma=0 (a useful correctness invariant, since
focal loss is a strict generalization of CE)."""
...
def test_focal_loss_downweights_easy_examples():
    """A confidently-correct prediction should contribute much less to focal
    loss than to plain CE, since (1-p_t)^gamma -> 0 as p_t -> 1.

    Logits are "confident but not extreme" (p_t ~= 0.987, not ~1.0) so
    neither loss underflows to exactly 0.0 in float32 — [10, -10, -10]
    saturates log_softmax hard enough that both losses round to identical
    exact zeros, which trivially (and misleadingly) satisfies a `<` check
    only if it doesn't also fail it; picking values just past float32's
    resolution here keeps the comparison actually meaningful.
    """
    torch.manual_seed(0)
    easy_logits = torch.tensor([[4.0, -1.0, -1.0]])  # near-certain correct prediction for class 0
